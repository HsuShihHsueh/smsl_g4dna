...
cp ../2.Mdgx/${drug}.amb2gmx/*${drug}* ./
cp ../4.Align/${drug}_GMX.align.gro.pdb ./${drug}_GMX.gro.pdb
cp ../4.Align/${pdbid}.pgro.pdb ./

## 合併G四聯體與藥物
cat ${pdbid}.gro.pdb    > ${complex}.gro.pdb ## ${pdbid}.gro.pdb 由 gmx pdb2gmx -f ${pdbid}.pgro.pdb  -o ${pdbid}.gro.pdb 轉換而來，更換成force field 的 atomname
cat ${drug}_GMX.gro.pdb >> ${complex}.gro.pdb
# Add 3 `TER` between `DNA (TER) K (TER) Drug (TER) END` by maunally

## Modify ${drug}_GMX.top
